...
check_timeout() {
    ...
    if [ "$elapsed" -gt "$WAIT_TIMEOUT" ]; then
        echo "[entrypoint] timeout após ${elapsed}s esperando por $1" >&2
        exit 1
    fi
}

echo "[entrypoint] aguardando PostgreSQL (timeout ${WAIT_TIMEOUT}s)..."
...
echo "[entrypoint] PostgreSQL pronto ($(get_elapsed)s)"

echo "[entrypoint] aguardando Redis (timeout ${WAIT_TIMEOUT}s)..."
if [ -n "$REDIS_URL" ]; then
...
echo "[entrypoint] Redis pronto ($(get_elapsed)s)"
...
echo "[entrypoint] pronto — iniciando Gunicorn"
